...
export class DevicesSrvService {
 // Define API
  ...
  createDevice(device:Device): Observable<Device> {
    return this.http.post<Device>(this.apiURL, JSON.stringify(device), this.httpOptions)
    .pipe(
      tap(data => {// Do your success stuff in here
    this.toastr.success('Success', 'device created',
    {timeOut: 2000});
      }),
     // retry(1),
     // catchError(this.handleError)
  ...
  updateDevice(id:number, device:Device): Observable<Device> {
    return this.http.put<Device>(this.apiURL + '/' + id +'/', JSON.stringify(device), this.httpOptions)
    .pipe(
      tap(data => {// Do your success stuff in here
    this.toastr.success('Success', 'device updated',
    {timeOut: 2000});
      }),
     // retry(1),
     // catchError(this.handleError)
  ...
  deleteDevice(id:number){
    return this.http.delete<Device>(this.apiURL + '/' + id, this.httpOptions)
    .pipe(
      tap(data => {// Do your success stuff in here
    this.toastr.success('Success', 'device deleted',
    {timeOut: 2000});
      }),
      //retry(1),
     // catchError(this.handleError)
