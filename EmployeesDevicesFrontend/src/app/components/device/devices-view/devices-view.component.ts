...
export class DevicesViewComponent implements OnInit {
  ...
  displayedColumns1: string[] = ['id','serial_number','description','type','owner','edit','delete'];
  dataSource1: any=[];
  ...
  constructor(private devSrv:DevicesSrvService,private empSrv:EmployeesSrvService,private toastr: ToastrService,private router: Router) { }

  ngOnInit(): void {
    this.devSrv.getDevices().subscribe((res: Device) => { this.dataSource1 = res; }, (error) => {
      this.toastr.warning('Error', error.message + " " + error.error.serial_number + " " + error.error.description,
        { timeOut: 4000 });
    });
  ...
  DeleteDev(id:number){
    this.devSrv.deleteDevice(id).subscribe((res) => {
      this.devSrv.getDevices().subscribe((res: any) => { this.dataSource1 = res;},
